...
public class DailyIndicatorCountRepositoryTest {
    ...
    public void findTalliesInMonthWhenGivenValidMonthDateInYYYYMMShouldReturn10Tallies() throws ParseException {
        ...
        Mockito.doReturn(matrixCursor)
                .when(sqLiteDatabase)
                .rawQuery(
                        ArgumentMatchers.eq("SELECT indicator_daily_tally._id, indicator_daily_tally.indicator_code, indicator_daily_tally.indicator_value, indicator_daily_tally.indicator_value_set, indicator_daily_tally.indicator_grouping, indicator_daily_tally.indicator_is_value_set, indicator_daily_tally.day, indicator_queries.expected_indicators FROM indicator_daily_tally INNER JOIN indicator_queries ON indicator_daily_tally.indicator_code = indicator_queries.indicator_code WHERE indicator_daily_tally.day >= ? AND indicator_daily_tally.day <= ?")
    ...
    public void getIndicatorTalliesForDayShouldReturn8TalliesWhenGivenValidDateWithTallies() throws ParseException {
        MatrixCursor matrixCursor = new MatrixCursor(new String[]{"_id", "indicator_code", "indicator_value"
                , "indicator_value_set", "indicator_is_value_set", "indicator_grouping", "day", "expected_indicators"}, 1);
        matrixCursor.addRow(new Object[]{1, "ME_Child_HIV_Status_Under2_Gender", null, "[[\"hiv_status\",\"gender\",\"counter\"],[\"HIV Unknown\",\"Male\",1]]", 1, null, "2017-03-10", "[\"ME_Child_HIV_Status_Under2_Gender_HIV Exposed_Female\",\"ME_Child_HIV_Status_Under2_Gender_HIV Exposed_Male\",\"ME_Child_HIV_Status_Under2_Gender_HIV Negative_Female\",\"ME_Child_HIV_Status_Under2_Gender_HIV Negative_Male\",\"ME_Child_HIV_Status_Under2_Gender_HIV Positive_Female\",\"ME_Child_HIV_Status_Under2_Gender_HIV Positive_Male\",\"ME_Child_HIV_Status_Under2_Gender_HIV Unknown_Female\",\"ME_Child_HIV_Status_Under2_Gender_HIV Unknown_Male\"]"});
        matrixCursor.addRow(new Object[]{1, "ME_Child_Total", 4, null, 0, null, "2017-03-10", null});
        ...
        Mockito.doReturn(matrixCursor)
                .when(sqLiteDatabase)
                .rawQuery(
                        ArgumentMatchers.eq("SELECT indicator_daily_tally._id, indicator_daily_tally.indicator_code, indicator_daily_tally.indicator_value, indicator_daily_tally.indicator_value_set, indicator_daily_tally.indicator_grouping, indicator_daily_tally.indicator_is_value_set, indicator_daily_tally.day, indicator_queries.expected_indicators FROM indicator_daily_tally INNER JOIN indicator_queries ON indicator_daily_tally.indicator_code = indicator_queries.indicator_code WHERE indicator_daily_tally.day = '2017-03-10'")
